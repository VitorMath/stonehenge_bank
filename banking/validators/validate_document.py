"""
	Module containing the validate_document function.
"""
...
def validate_document(self, data):
    """
    This method validates that the numbers are valid and follow the
    standard for each type of person.
    """
    ...
    if not document_number.isnumeric():
        raise serializers.ValidationError(
            {"Document Number": "Please, type just numbers."}
        )

    if (len(document_number) != 11) and (len(document_number) != 14):
        raise serializers.ValidationError(
            {"Document Number": "Please, retype the document number."}
        )
    ...
    if person_type == "NP":  # NP = NATURAL_PERSON
        ...
        raise serializers.ValidationError(
            {"Document Number": "Invalid Natural Person Document. Please retype."}
        )
    ...
    if person_type == "LP":  # LP = LEGAL_PERSON
        ...
        raise serializers.ValidationError(
            {"Document Number": "Invalid Legal Person Document. Please retype."}
        )
